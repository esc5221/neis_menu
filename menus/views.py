    ...
    def get(self, request, school_id):
        ...
        if date is None:
            raise CustomException(
                detail="date is required", status_code=status.HTTP_400_BAD_REQUEST
            )
        if type is None:
            raise CustomException(
                detail="type is required", status_code=status.HTTP_400_BAD_REQUEST
            )

        menu = self.get_object_or_404(school_id=school_id, date=date, type=type)
...
class MenuWeeklyView(CustomView):
    ...
    def get(self, request, school_id):
        ...
        weekly_menu_queryset = self.model.objects.filter(
            school_id=school_id, date__gte=week_start_date, date__lte=week_end_date
        )
        ...
        for date, menu_object in weekly_menu_object_list_grouped_by_date:
            ...
            weekly_menu_list.append(
                MenuListOfDateSchema(date=date, menus=menu_schema_list)
            )

        return Response(
            self.get_response_data(weekly_menu_list, many=True),
