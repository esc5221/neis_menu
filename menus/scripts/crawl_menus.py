...
def run(*args):
    if args[0] == "test":
        ...
        try:
            ...
            count = 0
            school_infos = School.objects.values_list("code", "edu_office_code")
            for school_code, edu_office_code in school_infos:
                menu_list = get_menu_pydantic_model(
                    school_code, edu_office_code, start_date, end_date
                )
                for i in menu_list:
                    valid_data_dict = i.dict()
                    valid_data_dict["school"] = School.objects.get(code=school_code)
