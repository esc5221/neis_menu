...
class TestMenuAPI(TestCase):
    ...
    def test_get_menus_success(self):
        ...
        for i_type in range(1, 4):
            ...
            self.assertEqual(
                response.json()['dishes'], menu.dishes)
            self.assertEqual(
                response.json()['calories'], menu.calories)
    ...
    def test_response_success_with_date_specified(self):
        ...
        for menu in response.json()['items'][0]['menus']:
            self.assertEqual(
                MenuDTO(**menu),
                MenuDTO(
                    **getattr(self, f"menu_1_{3}_{7}_{menu['type']}").__dict__
                )
            )
    ...
    def test_response_success_this_week(self, mock_today):
        ...
        for menu in response.json()['items'][0]['menus']:
            self.assertEqual(
                MenuDTO(**menu),
                MenuDTO(
                    **getattr(self, f"menu_1_{8}_{22}_{menu['type']}").__dict__
                )
            )
