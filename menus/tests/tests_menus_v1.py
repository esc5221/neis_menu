...
class TestMenuView(TestCase):
                ...
                for i_date in range(1, 32):
                    ...
                    if date_is_in_weekday := (date.weekday() < 5):
                        for i_type in range(1, 4):
                            ...
                            self.assertEqual(response.data["dishes"], menu.dishes)
                            self.assertEqual(response.data["calories"], menu.calories)
    ...
    def test_response_no_date(self):
        date = datetime.date(year=2022, month=3, day=7)
        for i_type in range(1, 4):
            response = self.client.get(f"/api/v1/schools/1/menus/?type={i_type}")
...
class TestMenuWeeklyView(TestCase):
    ...
    @patch(
        "menus.views.MenuWeeklyView.get_today",
        return_value=datetime.date(year=2022, month=8, day=24),
    )
    def test_response_success_this_week(self, mock_today):
        for i in range(1, 4):
            ...
            date_str = date.strftime("%Y-%m-%d")
            response = self.client.get(f"/api/v1/schools/{school.id}/menus/weekly/")
            self.assertEqual(response.status_code, status.HTTP_200_OK)
            self.assertEqual(len(response.data["menu_list"]), 5)
