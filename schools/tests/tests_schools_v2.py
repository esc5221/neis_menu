...
class TestSchoolAPI(TestCase):
    ...
    def test_list_schools_success(self):
        ...
        for i in range(1, 4):
            ...
            self.assertEqual(
                response.json()["items"][i - 1]["code"],
                getattr(self, f"school_{i}").code,
            )
            self.assertEqual(
                response.json()["items"][i - 1]["location"],
                getattr(self, f"school_{i}").location,
            )
            self.assertEqual(
                response.json()["items"][i - 1]["address"],
                getattr(self, f"school_{i}").address,
            )
            self.assertEqual(
                response.json()["items"][i - 1]["type"],
                SchoolTypes(getattr(self, f"school_{i}").type).name,
            )
            self.assertEqual(
                response.json()["items"][i - 1]["edu_office_code"],
