...
class TestSchoolView(TestCase):
    ...
    def test_db_data(self):
        for i in range(1, 4):
            ...
            object = School.objects.get(id=school.id)
            self.assertEqual(object.name, school.name)
            self.assertEqual(object.code, school.code)
            self.assertEqual(object.location, school.location)
            self.assertEqual(object.address, school.address)
            self.assertEqual(object.type, school.type)
            self.assertEqual(object.edu_office_code, school.edu_office_code)
